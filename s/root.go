...
func (spec SupervisorSpec) rootStart(
	...
	childrenSpecs, rscCleanup := spec.buildChildrenSpecs()
	...
	sup := Supervisor{
		...
		wait: func(stopingTime time.Time, startErr error) error {
			// We make sure that once the wait call finishes, we cleanup all the
			// resources generated by the children construction function.
			//
